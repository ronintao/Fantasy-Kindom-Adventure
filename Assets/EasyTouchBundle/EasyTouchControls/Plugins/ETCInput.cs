...
public class ETCInput : MonoBehaviour{
    ...
    /// <summary>
    /// Get Button Down Time, return -1 if the button is not pressed
    /// </summary>
    public static float GetButtonDownTime(string buttonName) {
        if (ETCInput.instance.axes.TryGetValue(buttonName, out axis)
                && (axis.axisState == ETCAxis.AxisState.Down || axis.axisState == ETCAxis.AxisState.Press))
            return Time.time - axis.downTimeStamp;
