...
namespace PathologicalGames
{
    ...
    ///
    ///  * Instantiated objects can optionally be made a child of this GameObject
    ///    (reffered to as a 'group') to keep the scene hierarchy organized.
    ///
    ...
    ///
    ///  * Two events are implemented to enable objects to handle their own reset needs.
    ///    Both are optional.
    ...
    public partial class SpawnPool : MonoBehaviour, IList<Transform>
    {
        ...
        /// <summary>
        /// If set to true, the spawned gameobject will receive OnSpawned Event
        /// </summary>
        public bool ShouldBroadCastMessage = true;
        ...
        private void Awake()
        {
            ...
            if (this._dontDestroyOnLoad) DontDestroyOnLoad(this.gameObject);
        ...
        /// <summary>
        ///	See docs for Despawn(Transform instance) for basic functionality information.
        ///		
        ...
        public void DestroyPrefabPool(GameObject prefab)
        {
            ...
            if (prefabPool != null)
            {
                ...
                this.prefabs._Remove(prefab.name);
            }
        }

        public void DestroyAllUnUsedPrefabPool()
        {
            StopAllCoroutines();
            bool destroyPool = true;
            while (destroyPool)
            {
                destroyPool = false;
                for (int i = 0; i < this._prefabPools.Count; i++)
                {
                    ...
                    {
                        this._prefabPools[i].RemoveAllNullSpawnedInstances();
                        if (this._prefabPools[i]._spawned.Count == 0)
                        {
                            DestroyPrefabPool(_prefabPools[i].prefabGO);
                            destroyPool = true;
                            break;
                        }
                    }
    ...
    public partial class PrefabPool
    {
        ...
        {
            return DespawnInstance(xform, spawnPool.ShouldBroadCastMessage);
        }
        ...
        {
            // Ugly crash preventions. 
            if (this._spawned.Count > 0 && this._spawned[0] == null)
            {
                RemoveAllNullSpawnedInstances();
            }
            if (this._despawned.Count > 0 && this._despawned[0] == null)
            {
                RemoveAllNullDespawnedInstances();
            }
